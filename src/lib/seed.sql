...
INSERT INTO bluzettSystem_products (name, slug, sku, description, short_description, price, regular_price, sale_price, stock_status, stock_quantity, rating_count, average_rating, status)
VALUES (
...
FROM bluzettSystem_products p WHERE p.slug = 'asus-rog-strix-scar-18' ON CONFLICT DO NOTHING;

INSERT INTO bluzettSystem_product_images (product_id, src, name, alt, position)
SELECT p.id, 'https://images.unsplash.com/photo-1625336960423-228c02f71e4f?w=600', 'MSI Raider', 'MSI Raider GE78 HX', 0
FROM bluzettSystem_products p WHERE p.slug = 'msi-raider-ge78-hx' ON CONFLICT DO NOTHING;
...
UPDATE bluzettSystem_categories c
SET count = (
  SELECT COUNT(*) FROM bluzettSystem_product_categories pc WHERE pc.category_id = c.id
);